...
try:
  ...
  from contactopt.manopth.mano.webuser.smpl_handpca_wrapper_HAND_only \
    import load_model as load_mano_model
  MANO_PRESENT = True
except ImportError:
...
def default_argparse(p_num, intent, object_name):
  parser = argparse.ArgumentParser()
  parser.add_argument('--p_num', type=int,default=p_num, help='Participant number (1-50)')
  parser.add_argument('--intent', default=intent,  choices=('use', 'handoff'), help='Grasp intent')
  parser.add_argument('--object_name',default=object_name, help="Name of object")
  return parser
...
def draw_hands(im, joints, colors=((0, 255, 0), (0, 0, 255)), circle_radius=3,
               line_thickness=2, offset=np.zeros(2, dtype=np.int8)):
  if im is None:
    print('Invalid image')
    return im
...
def draw_object_markers(im, ms, color=(0, 255, 255), circle_radius=3,
                        offset=np.zeros(2, dtype=np.int8)):
  if im.ndim == 2:  # depth image
    im = colorcode_depth_image(im)
  for m in np.round(ms).astype(np.int8):
...
def crop_image(im, joints, crop_size, fillvalue=[0]):
  ...
  bbcenter = np.round(np.mean(joints, axis=0)).astype(np.int8)
  im_crop = np.zeros((crop_size, crop_size, im.shape[2]), dtype=im.dtype)
  tl = bbcenter - crop_size//2
  br = bbcenter + crop_size//2
  tl_crop = np.asarray([0, 0], dtype=np.int8)
  br_crop = np.asarray([crop_size, crop_size], dtype=np.int8)
