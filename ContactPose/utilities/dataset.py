...
class ContactPose(object):
  ...
  def __init__(self, p_num, intent, object_name, mano_pose_params=15,
               load_mano=True, base_dir='/remote-home/share/datasets/haoming/ContactPose'):
    """
    ...
    self.data_dir = osp.join(self.base_dir, 'data', 'contactpose_data', p_id, object_name)
    ...
    oM = np.loadtxt(osp.join(self.base_dir, 'data',
                             'object_marker_locations',
                             '{:s}_final_marker_locations.txt'.
                             format(object_name)))[:, :3]
    ...
    if mutils.MANO_PRESENT and load_mano:
      ...
      for hand_name in ('LEFT', 'RIGHT'):
        ...
        #
        filename = osp.join(self.base_dir, 'data', 'thirdparty',
                            'mano', 'models',
                            'MANO_{:s}.pkl'.format(hand_name))
