...
from contactopt.manopth.mano.webuser.smpl_handpca_wrapper_HAND_only import ready_arguments
from contactopt.manopth.manopth import rodrigues_layer, rotproj, rot6d
from contactopt.manopth.manopth.tensutils import (th_posemap_axisang, th_with_zeros, th_pack,
...
class ManoLayer(Module):
    ...
    def __init__(self,
        ...
        # self.register_buffer('th_betas',
        #                      torch.from_numpy(smpl_data['betas'].r.copy()).unsqueeze(0))
        # self.register_buffer('th_shapedirs',
        #                      torch.from_numpy(smpl_data['shapedirs'].r.copy()))
        # self.register_buffer('th_posedirs',
        #                      torch.from_numpy(smpl_data['posedirs'].r.copy()))
        # self.register_buffer('th_v_template',
        #                      torch.from_numpy(smpl_data['v_template'].r.copy()).unsqueeze(0))
        # self.register_buffer('th_J_regressor',
        #                      torch.from_numpy(np.array(smpl_data['J_regressor'].toarray()).copy()))
        # self.register_buffer('th_weights',
        #                      torch.from_numpy(smpl_data['weights'].r.copy()))
        # self.register_buffer('th_faces',
        #                      torch.from_numpy(smpl_data['f'].astype(np.int32).copy()).long())
