...
class HandObject:
    ...
    """
    closed_faces = util.get_mano_closed_faces(mano_root=osp.join(os.path.dirname(__file__),'manopth', 'mano', 'models'))
    ...
    def load_from_contactpose(self, cp_obj):
        ...
        self.obj_sem_contact = np.expand_dims(util.create_semantic_contact(obj_mesh, cp_obj), axis=1)

        self.obj_name = cp_obj.object_name
        self.obj_class = util.name_to_class(self.obj_name)
        self.p_num = cp_obj.p_num
        self.intent = cp_obj.intent
    ...
    def run_mano(self):
        ...
        if self.hand_pose.shape[0] == 48:   # Special case when we're loading GT honnotate
            mano_model = ManoLayer(mano_root=osp.join(osp.dirname(os.getcwd()),'contactopt','manopth','mano', 'models'), joint_rot_mode="axisang", use_pca=False, center_idx=None, flat_hand_mean=True)
        else:   # Everything else
            mano_model = ManoLayer(mano_root=osp.join(osp.dirname(os.getcwd()),'contactopt','manopth','mano', 'models'), use_pca=True, ncomps=15, side='right', flat_hand_mean=False)
