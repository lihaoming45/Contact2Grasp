...
def get_object_names(p_num, intent,data_path=None, ignore_hp=True):
  """
  ...
  if bool(data_path):
    sess_dir = osp.join(data_path,'contactpose_data', sess_dir)
  else:
    sess_dir = osp.join(osp.dirname(__file__), '..', 'data', 'contactpose_data', sess_dir)
  ignored_objects = ('hands', 'palm_print','bowl','binoculars') if ignore_hp else ()
  return [o for o in next(os.walk(sess_dir))[1] if o not in ignored_objects]


def get_intents(p_num, object_name,data_path=None):
  """
  ...
  for ins in ('use', 'handoff'):
    ...
    if bool(data_path):
      sess_dir = osp.join(data_path, 'contactpose_data', sess_dir, object_name)
    else:
...
def get_p_nums(object_name, intent,data_path):
  """
...
class ContactPose(object):
  ...
  def __init__(self, p_num, intent, object_name, data_path=None, mano_pose_params=15,
               load_mano=True):
    """
    ...
    if self.data_path is not None:
      self.data_dir = osp.join(self.data_path,'contactpose_data', p_id, object_name)
    else:
    ...
    if bool(self.data_path):
      oM = np.loadtxt(osp.join(self.data_path,
    ...
    if mutils.MANO_PRESENT and load_mano:
      ...
      for hand_name in ('LEFT', 'RIGHT'):
        if bool(self.data_path):
          filename = osp.join(self.data_path, 'thirdparty',
                              'mano', 'models',
